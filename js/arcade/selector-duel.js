...
const LEVELS = [
  { dom: '<div class="card"></div><div class="card hot"></div><div class="card"></div>', target: '.hot', hint: 'Select the .hot card' },
  { dom: '<button id="go">Go</button><button>Stop</button>', target: '#go', hint: 'Select the button with id="go"' },
  { dom: '<div><span></span><span class="x"></span><span></span></div>', target: '.x', hint: 'Select the middle span' },
  { dom: '<ul><li></li><li class="want"></li><li></li><li class="want"></li><li></li></ul>', target: '.want', hint: 'Select all .want items' },
  { dom: '<a href="#"></a><a href="https://x.com"></a><a href="#" class="ext"></a>', target: '[href^="https"]', hint: 'Select links with https hrefs' },
  { dom: '<input type="text"><input type="email"><input type="password">', target: '[type="email"]', hint: 'Select the email input' },
  { dom: '<div><p>1</p><p>2</p><p>3</p></div>', target: 'p:first-child', hint: 'Select only the first p' },
  { dom: '<div><p></p><p></p><p></p><p></p></div>', target: 'p:last-child', hint: 'Select only the last p' },
  { dom: '<ul><li></li><li></li><li></li><li></li><li></li></ul>', target: 'li:nth-child(odd)', hint: 'Select odd li items' },
  { dom: '<div class="card"><h2></h2><p></p></div><div class="card"><h2></h2></div>', target: '.card:has(p)', hint: 'Select cards that contain a p' },
  { dom: '<a href="#one"></a><a href="other.html"></a><a href="#two"></a>', target: '[href^="#"]', hint: 'Select hash anchors' },
  { dom: '<input type="checkbox"><input type="checkbox" checked><input type="checkbox">', target: ':checked', hint: 'Select the checked input' },
  { dom: '<button disabled></button><button></button><button disabled></button>', target: ':disabled', hint: 'Select disabled buttons' },
  { dom: '<form><label></label><input><label></label><input></form>', target: 'label + input', hint: 'Select inputs immediately after labels' },
  { dom: '<section><article></article><div></div><article></article></section>', target: 'section > article', hint: 'Select articles that are direct children of section' },
  { dom: '<a href="x.pdf"></a><a href="x.html"></a><a href="y.pdf"></a>', target: '[href$=".pdf"]', hint: 'Select pdf links' },
  { dom: '<p class="text">A</p><p class="info xtra">B</p><p class="text xreg">C</p>', target: '[class*="x"]', hint: 'Select elements whose class contains "x"' },
  { dom: '<div><div><span></span></div></div>', target: 'div div span', hint: 'Select the deeply nested span' },
  { dom: '<ul><li><a></a></li><li><a></a></li></ul>', target: 'li:not(:first-child) a', hint: 'Select link in all but first li' },
  { dom: '<input required><input><input required>', target: '[required]', hint: 'Select required inputs' },
  { dom: '<div lang="en"></div><div lang="fr"></div><div lang="de"></div>', target: '[lang="fr"]', hint: 'Select French element' },
  { dom: '<button class="btn primary"></button><button class="btn"></button>', target: '.btn.primary', hint: 'Select primary buttons' },
  { dom: '<div class="a b c"></div><div class="a"></div><div class="b c"></div>', target: '.a.b.c', hint: 'Select element with all three classes' },
  { dom: '<table><tr><td></td><td></td></tr><tr><td></td><td></td></tr></table>', target: 'tr:last-child td:first-child', hint: 'Bottom-left table cell' },
  { dom: '<details open><summary>S</summary><p>Hi</p></details>', target: 'details[open] p', hint: 'Select p inside open details' },
  { dom: '<div class="grid"><div></div><div></div><div></div><div></div></div>', target: '.grid > div:nth-child(2n)', hint: 'Even grid children' },
  { dom: '<header></header><main></main><footer></footer>', target: 'main', hint: 'Select main element' },
  { dom: '<dialog></dialog><dialog open></dialog>', target: 'dialog[open]', hint: 'Select open dialog' },
  { dom: '<img alt=""><img alt="cat"><img>', target: 'img[alt=""]', hint: 'Select image with empty alt attribute (decorative)' },
  { dom: '<p>1</p><p>2</p><p>3</p>', target: 'p', hint: 'Select all paragraphs' },
];
